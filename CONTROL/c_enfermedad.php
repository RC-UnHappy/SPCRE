<?php 
	...
	$rsMetodo = consultarMetodos('Enfermedad');
	$sI = '1';
	$sM = '1';
	$sC = '1';
	$sE = '1';
	...
	$objEnf = new cls_enfermedad();

	function listar_enf(){ # al cargar la pagina se ejecuta esta función, ver: lista_enfermedades.php
		global $objEnf;
		$rs = $objEnf->listar_limit();
	...
	if( isset($_POST['ope']) ){ # existe variable POST ope del formulario
		$objEnf->set_datos( $_POST['cod'],$_POST['nom'] );
		switch ($_POST['ope']) {
			case 'mod':
				if( $objEnf->modificar() ){
					header('location: ../VISTA/?ver=enfermedad&mod=true');
				}else{
					header('location: ../VISTA/?ver=enfermedad&error=nombre');
				}
				break;
			
			case 'add':
				if( $objEnf->agregar() ){
					header('location: ../VISTA/?ver=enfermedad&add=true');
				}else{
					header('location: ../VISTA/?ver=enfermedad&error=nombre');
				}
				break;
			...
			case 'elm':
				if( $objEnf->eliminar() ){
					header('location: ../VISTA/?ver=enfermedad&elm=true');
				}
				break;
		}
	}
	...
	else if( isset($_POST['listar']) ){ # listar enfermedades todas
		$rs = $objEnf->listar_limit($_POST['desde'],$_POST['mostrar']);
		echo $objEnf->filas.'%'; 
		crea_lista($rs);
	}

	else if( isset($_POST['filtrar'] ) ){ # lista enfermedades por filtro
		$objEnf->set_filtro($_POST['filtro']);
		$rs = $objEnf->filtrar($_POST['desde'],$_POST['mostrar']);
		echo $objEnf->filas.'%'; 

		if( $objEnf->filas > 0 && !$rs ){ 
			$rs = $objEnf->filtrar( 0, $_POST['mostrar']); # desde = 0;
			crea_lista($rs, true);
		}
		else{
			crea_lista($rs);
	...
	function paginas($pag_actual=1, $limite=15){ # Numeracion de páginas
		global $objEnf;
		$total = $objEnf->filas; # Ej: 30 filas
