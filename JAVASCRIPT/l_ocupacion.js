window.onload = function(){ // carga los eventos una vez la pagina  haya cargado
	controlador = '../CONTROL/c_ocupacion.php';

	// VENTANAS
	divForm = document.getElementById('form-ocup');
	...
	if( document.getElementById('txt_buscar') ){
		...
		txt_buscar.onkeyup = searchOcupacion;
		btn_buscar =  document.getElementById('btn_buscar');
		btn_buscar.onclick = searchOcupacion; 
	}
	...
	// FORMULARIOS
	f = document.f_ocupacion; // formulario
	f.enviar.onclick = enviar_formulario;	
	f.nom.onchange = validar_nom; f.nom.onkeypress = solo_letras;
	f.ope.value = 'add';
...
function W_default(){
	...
	f.reset();
	ocultar_msj(msjBox);
	document.getElementById('W-nom').innerHTML = '<i class="icon-plus"></i>Agregar ocupación';
...
function W_OpenMod(cod,nom){
	W_open();
	f.cod.value = cod;
	f.ope.value = 'mod';
	f.nom.value = nom.replace(/_s_/g,' ');
	document.getElementById('W-nom').innerHTML = '<i class="icon-edit"></i>Modificar ocupación: '+cod;
...
function urlVarGet(){
	...
	p = '<p class="msj_error"> El nombre de el ocupación ya se encuentra registrado.</p>';
	...
	if( vError = getVariable('error') ){
		OpenWindowNot(msj3+p); // muestra alerta
	}
}