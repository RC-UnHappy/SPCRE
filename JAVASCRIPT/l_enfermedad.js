window.onload = function(){ // carga los eventos una vez la pagina  haya cargado
	...
	if( document.getElementById('txt_buscar') ){
		...
		txt_buscar.onkeyup = searchEnf;
		btn_buscar =  document.getElementById('btn_buscar');
		btn_buscar.onclick = searchEnf; 
	}
	...
	// FORMULARIO
	f = document.f_enfermedad; // formulario
...
function W_default(){
	...
	f.reset();
	ocultar_msj(msjBox);
	document.getElementById('W-nom').innerHTML = '<i class="icon-plus"></i>Agregar enfermedad';
...
function W_OpenMod(cod){ 
	...
	f.ope.value = 'mod';
	f.cod.value = cod;
	f.nom.value = document.getElementById(cod+'-nomEnf').innerText;
	document.getElementById('W-nom').innerHTML = '<i class="icon-edit"></i>Modificar enfermedad';
...
function urlVarGet(){ // busca variables en la URL
	...
	msj4 = '¡Enfermedad eliminada! <i class="icon-ok-circled2"></i>';
	p = '<p class="msj_error"> El nombre de la enfermedad ya se encuentra registrada.</p>';
	if( vReg = getVariable('add') ){ // existe variable get add
